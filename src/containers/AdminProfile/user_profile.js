...
class UserProfile extends Component {
	...
	componentDidUpdate(prevProps) {
		if (!_.isEqual(this.props.adminUser, prevProps.adminUser)) {
			this.setOptions()
		}
	}

	setOptions = () => {
		this.setState({ adminUser: this.props.adminUser }, this.setFormInputs)
	}
	...
	onSubmit = (values) =>{
		...
		return updateUser(newUserProfile).then(res => {
	...
	setFormInputs = () => {
		...
		const user_profile = adminUser.profile || {}
...
const mapStateToProps = state => {
	return {
		adminUserId: state.FB.companyUser.userId,
		adminUser: state.FB.companyUser.user,
	}
}
