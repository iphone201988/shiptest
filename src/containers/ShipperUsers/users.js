...
class Users extends PureComponent {
    ...
    disableUser = (user) => {
        ...
        if (user instanceof ShipperUser){
            disableUser({userId: user.id}).then((res)=> {
                Notification('success', this.props.intl.formatMessage({id: "shipper.user.delete_success"}))
            }).catch(e=>{
                Notification('error', this.props.intl.formatMessage({id: "shipper.user.delete_fail"}))
            })
        }
    }
    ...
    setFilterConditions = (formValues={}) => {
        ...
        this.setState(setUserFilter(formValues, view, this.props.companyId, "shipper_user"))
    }

    render() {
        let {viewType, columns, queryFilterConditions, resultsFilterConditions, itemActions, itemType} = this.state
