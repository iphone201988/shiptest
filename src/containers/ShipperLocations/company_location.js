...
class CompanyCompanyLocations extends Component {
  ...
  constructor(props) {
    ...
    this.state.View =  props.View || this.state.View
  ...
  setFilterConditions = (formValues={}) => {
    this.setState(setLocationFilter(formValues, this.props.companyId))
  }
  ...
  render() {
    const {queryFilterConditions, resultsFilterConditions, itemActions, viewType, zoom_to, columns } = this.state
    ...
    let content =
        <LayoutWrapper>
          ...
          {queryFilterConditions.length > 0 ?
              <DataView itemType={"shipper_company_location"}
                  ...
                  layouts={[{key: 'hybrid', sections:[
                      {type: 'table', span: 8},
                      {type: 'map', map_type: 'company_locations', span: 16, zoom_to:zoom_to, width: "100%", height: "70vh"}
                    ]
                  }]
                  }
              /> : ""}
...
CompanyCompanyLocations.propTypes = {
...
)(injectIntl(CompanyCompanyLocations))
